...
def line_annotate(axe, liste, x, y, text, xtext = 0, ytext = 0):
    axe.plot([x,x], [y, liste.min()-1], linewidth=1.5, linestyle='--', color = 'black')
...
if __name__ == "__main__":
    ...
    liste_d3 = np.zeros(size)
    liste_dm3 = np.zeros(size)
    liste_d = np.zeros(size)
    liste_time_min_QCMD = np.zeros(size)
    liste_lisse_time = np.zeros(2*size)
    liste_lisse_d = np.zeros(2*size)

    #get data from Excel
    time_min(liste_time_min, feuille, size)
    # df3(liste_df3, feuille, size)
    # dD3(liste_dD3, feuille, size)

    for i in range(size):
        liste_d[i] = (feuille['∆d(nm)'].iloc[i])
        liste_d3[i] = (feuille['∆d(nm) Sauerbrey'].iloc[i])
        liste_dm3[i] = (feuille['∆m3'].iloc[i])
        liste_time_min_QCMD[i] = (feuille['Time (décalé)'].iloc[i])/60
        liste_time_min[i] = (feuille['Time_1 [s]'].iloc[i])/60

    for i in range(2*size-1):
        if(i % 2 == 0):
            indice = int(i/2)
            liste_lisse_time[i] = liste_time_min[indice]
            liste_lisse_d[i] = liste_d[indice]
        else:
            indice = int((i-1)/2)
            liste_lisse_time[i] = (liste_time_min[indice]+liste_time_min[indice+1])/2
            liste_lisse_d[i] = (liste_d[indice]+liste_d[indice+1])/2
    #make a figure y=f(x)
    ...
    #ellipsometry
    #plt.plot(liste_time_min, liste_d, color='black',  linestyle='', marker='s', markersize=0.8)
#     plt.plot(liste_lisse_time, liste_lisse_d, color='black',  linestyle='',
# marker='s', markersize=0.8, label='Ellipsometry')
    #QCMD
    print(liste_time_min)
    #print(liste_dm3)
    plt.plot(liste_time_min, liste_dm3, color='red',  linestyle='none',
 marker='s', markersize=0.8)
    #Annotate the points with a line: e.g. at 10 and 60 min
    line_annotate(plt, liste_dm3, x1, y1, r'water', xtext = xtext1, ytext = ytext1)
    line_annotate(plt, liste_dm3, x2, y2, r'PVP', xtext = xtext2, ytext = ytext2)

    #plt.xlim(0, 60)
    plt.ylabel(r'$\rho$ d (ng.cm$^{-2}$)', fontsize=14, color='black')
    ...
    # axis[0].plot(liste_time_min, liste_df3, color='black',  linestyle='none',
    ...
    ax2 = axis[0].twinx()
    ax2.plot(liste_time_min, liste_dD3, color = 'red')
    ...
    #compute fitted data
    compute_fit_data(y_theo, liste_time_min, var[0], var[1], size_fit)
